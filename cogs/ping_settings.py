...
class PingSettingsView(discord.ui.View):
	...
	"""
	def __init__(self, user:User, *items: Item, timeout: float | None = 180, disable_on_timeout: bool = False) -> None: # type: ignore
		...
		self.toggleBtn:discord.Button = discord.ui.Button( # type: ignore
			style=discord.ButtonStyle.red if user.allowPing else discord.ButtonStyle.green,
			label=f'(test)// {"disable" if user.allowPing else "enable"} pings',
...
class PingRuleModal(discord.ui.Modal):
	def __init__(self, parent:PingSettingsView, *children: InputText, title: str, custom_id: str | None = None, timeout: float | None = None) -> None:
	...
	async def cb(self, interaction:Interaction) -> None:
		...
		for c in components[:2]:
			...
			if re.match(r'([0-1]\d|2[0-3]):[0-6]\d', timeString) is not None:
				...
				now:dt=dt.now()
				times.append(dt(now.year,now.month,now.day,*[int(ts) for ts in timeString.split(':')], tzinfo=tz).timetz())
			else:
...
class PingSettingsCog(commands.Cog):
	...
	async def lobbyPingSettings(self, ctx:discord.ApplicationContext) -> None:
		...
		await ctx.respond(embed=generateRulesEmbedded(user), view=PingSettingsView(user), ephemeral=True) # type: ignore
		user.save()
