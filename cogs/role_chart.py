...
class RoleChartCog(commands.Cog):
	...
	async def listUserInvites(self, ctx:discord.Message) -> None:
		...
		labels: list[str] = [f"[{roles[r].name}] {data[r]} / {round((data[r]/ctx.guild.member_count)*100),2}%" for r in range(len(roles))] # type: ignore
		colors: list[str] = ['#{0:02x}{1:02x}{2:02x}'.format(*r.color.to_rgb()) for r in roles]

		ax.pie([d/ctx.guild.member_count for d in data], labels=labels, colors=colors) # type: ignore
